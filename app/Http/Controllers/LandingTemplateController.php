...
class LandingTemplateController extends AppBaseController{
	...
	public function edit($id, Request $request){
		...
		if(!is_null($template->options) && $template->options != 'null'){
			$t_options = json_decode($template->options, true);
	...
	public function redirect($uuid){
		...
		$landingTemplate = LandingTemplate::where('uuid', $uuid)->first();
		...
		if(!is_null($landingTemplate->options) && $landingTemplate->options != 'null'){
			$options = json_decode($landingTemplate->options, true);
		}

		return view('landing_templates.landing', compact('content', 'options'));
	}
}
